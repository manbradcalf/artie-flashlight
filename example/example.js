(function ($) {
  ...
  function buildQuery($form) {
    ...
    var index = $form.find('[name="index"]:checked').val();
  ...
  function doSearch(query) {
    ...
    const URL = `https://dogwood-9512546.us-east-1.bonsaisearch.net/${query.index}/_search?q=${query.term}`

    $.get(URL, function(data, status){

        // Initialize results string
        var results = '';
        Object.keys(data.hits.hits).forEach(key => {
          // Add each item in hits array to results
          var result = JSON.stringify(data.hits.hits[key]._source)
          results += result + "\n"
